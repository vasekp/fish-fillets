...
AudioStream::AudioStream(Instance* instance) {
    ...
    do {
        ...
        if(outIndex >= 0) {
            ...
            LOGD("codec: buffer size %d", info.size);
            ...
            LOGD("out: try again later");
        } else if(outIndex == AMEDIACODEC_INFO_OUTPUT_BUFFERS_CHANGED) {
            LOGD("out: buffer changed");
        } else if(outIndex == AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED) {
            LOGD("out: format changed");
        } else
    ...
    auto min = std::reduce(dataF.get(), dataF.get() + numSamples, 0.f, [](auto x, auto y) { return std::max(x, y); });
    auto max = std::reduce(dataF.get(), dataF.get() + numSamples, 0.f, [](auto x, auto y) { return std::min(x, y); });
    LOGD("%f -- %f", min, max);
