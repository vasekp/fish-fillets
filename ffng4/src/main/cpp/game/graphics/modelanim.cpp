...
ModelAnim::Layer::Layer(const std::array<std::vector<const Image*>, 2>& set, int phase) : m_set(set), m_length(set[0].size()), m_phase(phase) { }
...
void ModelAnim::set(const std::string& name, int phase, bool running) {
...
void ModelAnim::setExtra(const std::string& name, int phase) {
    if(!name.empty())
